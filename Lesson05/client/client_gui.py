...
class MainApp(QMainWindow):
    ...
    def refresh_window(self):
        ...
        self.statusBar().showMessage(f'Текущий пользователь: {self.client_engine.username}, текущий чат: {self.current_user}')
    ...
    def show_known_users(self):
        ...
        for item in self.client_db.session.query(self.client_db.KnownUsers).all():
            ...
            if item.name in self.client_engine.online_userslist:
                ...
                table_item.append(QStandardItem('offline')) # Потом здесь будет семафорчик, зеленый - online
                                                        # красный - offline
    ...
    def delete_user(self):
        if self.current_user and QMessageBox.question(self, 'Подтвердите удаление записи',
